//
//  simd-block-modified.h
//  
//
//  Created by Ludovic Theobald on 6/7/2561 BE.
//

#ifndef simd_block_modified_h
#define simd_block_modified_h


#endif /* simd_block_modified_h */
...
public:
    ...
    explicit SimdBlockFilter(const int num_buckets_);
    SimdBlockFilter(SimdBlockFilter&& that)
    : num_buckets_(that.num_buckets_),
    ...
    uint64_t SizeInBytes() const { return sizeof(Bucket) * num_buckets_; }
...
SimdBlockFilter<HashFamily>::SimdBlockFilter(const int num_buckets_)
...
// too large.
directory_mask_((1ull << ::std::min(63, num_buckets_)) - 1), // ! ne sert à rien en l'état
directory_(nullptr),
hasher_() {
    if (!__builtin_cpu_supports("avx2")) {
        throw ::std::runtime_error("SimdBlockFilter does not work without AVX2 instructions");
    }
    const size_t alloc_size = num_buckets_ * LOG_BUCKET_BYTE_SIZE;
...
SimdBlockFilter<HashFamily>::Add(const uint64_t key) noexcept {
    ...
    const uint32_t bucket_idx = hash % num_buckets_;
